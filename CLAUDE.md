...
## What It Does
IconForge removes backgrounds from images and converts them to icon-ready formats (WebP, monochrome silhouette SVG via Potrace, or color multi-path SVG via VTracer). It has both a CLI and a web UI.

## Data Flow
```
Upload image → Crop (8px grid snap) → Background Removal (mandatory)
                ↓
     pick ONE output type (WebP | Silhouette SVG | Color SVG) → Process → Export
```
...
│   │   └── routes/                   # upload, crop, background, svg, color_svg, export, images, settings
│   └── core/
...
| settings.py    | `GET/PUT /api/settings/svg`, `GET/PUT /api/settings/color-svg`                                |
...
3. Vector outputs are **SVG** — either monochrome silhouette (Potrace) or multi-path color (VTracer)
4. Background removal is **mandatory** before any output type in the web UI; all downstream pipelines consume the BG-removed PNG from `backend/background_remover/output/`
